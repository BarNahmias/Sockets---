...
class Server:
    ...
    time_out = 0.001 # time out for socket rcv
    ...
    * third - server will wait for client ack
    ...
    def three_way_handshake(self, nick_name, syn):
        print(self.server_sock_udp_map.get(nick_name))
        while True:
            try:
                ...
                if message.decode('utf-8') == 'SYN' or syn:
                    print("inside")
                    syn = True
                    self.server_sock_udp_map.get(nick_name)[2].sendto('ACK'.encode('utf-8'),('127.0.0.1', self.server_sock_udp_map.get(nick_name)[0]))
    ...
    """""
    def close_connection(self, nick_name):
        try:
            self.server_sock_udp_map.get(nick_name)[2].settimeout(None)
            ans, _ = self.server_sock_udp_map.get(nick_name)[2].recvfrom(self.max_packet_size)
            self.server_sock_udp_map.get(nick_name)[2].settimeout(self.time_out)
            ...
            self.close_connection_2(nick_name)
            return

    def close_connection_2(self, nick_name):
        try:
            self.server_sock_udp_map.get(nick_name)[2].settimeout(None)
            self.server_sock_udp_map.get(nick_name)[2].sendto('FIN'.encode('utf-8'), ('127.0.0.1',self.server_sock_udp_map.get(nick_name)[0]))
            ans, _ = self.server_sock_udp_map.get(nick_name)[2].recvfrom(self.max_packet_size)
            if ans.decode('utf-8') == 'ACK':
        ...
        while True:
            try:
                ...
                if ack.decode('utf-8') == 'FIN':
                    self.close_connection(nick_name)
                    return 0
                to_send = 'SIZE:' + str((len(nack)-1))
                if ack.decode('utf-8') == 'SEND_SIZE':
                    self.server_sock_udp_map.get(nick_name)[2].sendto(to_send.encode('utf-8'),('127.0.0.1', self.server_sock_udp_map.get(nick_name)[0]))
                for i in range(start_index, end_index):
                    if i % 10 == int(ack.decode('utf-8')) % 10:
                        nack[i] = 1
                        if float(precent) > 0:
                            print(f'Send {precent}% from the file! To: {nick_name}')
        ...
        while end_index <= len(nack_packet):
            ...
            for k in range(start_index, end_index):
                if nack_packet[k] == 0:
                    ...
                    packet = ind+data
                    self.server_sock_udp_map.get(nick_name)[2].sendto(packet, ('127.0.0.1', self.server_sock_udp_map.get(nick_name)[0]))
            precent = (start_index / len(nack_packet)) * 100
            self.server_sock_udp_map.get(nick_name)[2].settimeout(self.time_out)
            precent = "{:.2f}".format(precent)
            if self.check_ack(nack_packet, start_index, end_index, precent, nick_name) == 0:
                print(f'closed connection with client : {nick_name}')
            ...
            if bool_precent and float(precent) > 40:
                ...
                while True:
                    ...
                    try:
                        ...
                        self.server_sock_udp_map.get(nick_name)[2].sendto('CONFIRM_PROCEED'.encode('utf-8'),('127.0.0.1', self.server_sock_udp_map.get(nick_name)[0]))
    ...
    def handle_messages(self, nick_name):
        while True:
            try:
                ...
                print(message)
                # if clieant exit this opertion relise her ports and free all the place that the client save .
...
if __name__ == "__main__":
    ...
    server.file_names.append("MP4.mp4")
    server.file_names.append("photo.jpeg")
